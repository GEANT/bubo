        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=10) as response:
                if response.status != 200:
                    logger.error(
                        f"Failed to fetch IANA parameters: HTTP {response.status}"
                    )
                    return None
...
def get_cached_info(cache_file: str) -> Tuple[Optional[datetime], Optional[datetime]]:
    """
...
def update_cache_info(
    cache_file: str,
    iana_date: Optional[datetime] = None,
...
async def download_iana_csv(csv_url: str, save_path: str) -> bool:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(csv_url, timeout=10) as response:
                if response.status != 200:
