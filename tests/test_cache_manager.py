...
def test_save_and_get_results(cache_manager, sample_results):
    ...
    assert cached_results["validations"]["RPKI"]["state"]["domain.com"]["Nameserver of Domain"] == "valid"

    # Pattern matching for structure
    assert all(standard in cached_results["validations"]
               for standard in ["RPKI", "DANE", "DNSSEC"])
    assert "domain_metadata" in cached_results
    assert isinstance(cached_results["validations"]["DNSSEC"]["state"][domain]["DNSSEC"], bool)
...
def test_cache_duration_custom(cache_dir):
    ...
    cache_manager = DomainResultsCache(cache_dir=str(cache_dir), cache_duration=custom_duration)
...
def test_multiple_domains(cache_manager, sample_results):
    ...
    for domain in domains:
        ...
        modified_results["domain_metadata"] = {domain: {"country": "US", "institution": "Example"}}
...
def test_cache_update(cache_manager, sample_results):
    ...
    updated_results["validations"]["RPKI"]["state"][domain]["Nameserver of Domain"] = "not-valid"
    cache_manager.save_results(domain, updated_results)
    cached_results = cache_manager.get_results(domain)
    assert cached_results["validations"]["RPKI"]["state"][domain]["Nameserver of Domain"] == "not-valid"
