...
async def test_resolve_nameservers_success(
    mock_dns_resolver, mock_ns_records, sample_domain
):
    with (
...
async def test_resolve_ips_success(
    mock_dns_resolver, mock_ipv4_records, mock_ipv6_records
):
    with patch(
        "dns.asyncresolver.Resolver.resolve",
        side_effect=[mock_ipv4_records, mock_ipv6_records],
    ):
...
async def test_resolve_ips_no_ipv6(mock_dns_resolver, mock_ipv4_records):
    with patch(
        "dns.asyncresolver.Resolver.resolve",
        side_effect=[mock_ipv4_records, dns.resolver.NoAnswer],
    ):
        ipv4, ipv6 = await resolve_ips("ns1.example.com")
        assert ipv4 == ["192.168.1.1"]
        assert ipv6 == ["No IPv6"]
